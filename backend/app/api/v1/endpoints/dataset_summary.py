...
_store: CumulativeStore | None = None
...
def clear_dataset_cache():
    """Invalidate summary & analytics cache."""
...
def _load_parquet() -> pd.DataFrame | None:
    """Return the cumulative DataFrame from CumulativeStore."""
    try:
        return _get_store().load_cumulative()
    except FileNotFoundError:
        return None
    except Exception as e:
        logger.warning(f"CumulativeStore load failed: {e}")
        return None
