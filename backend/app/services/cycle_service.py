...
from app.ingestion.schema_adapter import adapt
...
def _stage1_ingest_validate(
...
) -> tuple[StageResult, pd.DataFrame, list[str]]:
    """
    Stage 1: Ingest and validate.

    Order of checks:
    1. schema_adapter.adapt() — resolve DataCo aliases, 422 on missing required cols
    2. validate_continuity()  — overlap/gap/duplicate/target checks, 422 on REJECT
    3. period == next_period() continuity guard
    4. Intra-upload duplicate Order Item Id dedup

    Returns (StageResult, cleaned_df, continuity_warnings).
    Raises HTTPException(422) on any hard failure.
    """
    ...
    # 1. Schema adapter
    df, schema_report = adapt(df)
    if not schema_report.ok:
        raise HTTPException(
            status_code=422,
            detail=f"Stage 1 FAILED: missing required columns after schema adaptation: {schema_report.missing}",
        )
    ...
    try:
        ...
        cont_report = validate_continuity(df, df_cumulative, claimed_period=period)
        continuity_warnings = cont_report.warnings
        if continuity_warnings:
            logger.warning(f"Stage 1 continuity warnings: {continuity_warnings}")
    except ContinuityError as exc:
        raise HTTPException(status_code=422, detail=f"Stage 1 FAILED: {exc}")
    ...
    dup_count = 0
    id_col = next((c for c in ("Order Item Id", "order_item_id") if c in df.columns), None)
    if id_col:
    ...
    return StageResult(
        ...
        detail={
            ...
            "schema_resolved":     len(schema_report.resolved),
            "continuity_warnings": len(continuity_warnings),
        },
    ), df, continuity_warnings
...
def _stage3_compute_metrics(
    ...
    period: str,
) -> tuple[StageResult, dict[str, float] | None]:
    """
    ...
    if not forecast_exists:
        ...
        return StageResult(
            ...
            status="SKIPPED",
            duration_ms=duration_ms,
            detail={"reason": f"no standing forecast for {period}"},
        ), None
...
def _stage5_store_and_retrain(
    ...
    try:
        ...
        training_results = orchestrator.train_all(
            df_full, dataset_version=f"cumulative_{period}",
            already_engineered=True, training_path="other",
        )
