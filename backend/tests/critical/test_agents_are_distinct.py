    ...
    for agent_type in [IntelligenceType.SUPPLIER, IntelligenceType.LOGISTICS]:
        try:
            ...
            if "monotonic_cst" in str(e):
                pytest.skip(
                    f"Saved model was trained with an older sklearn version "
                    f"incompatible with the current install ({e}). "
                    f"Re-run initialization to retrain: python scripts/run_initialization.py --force"
                )
            raise
