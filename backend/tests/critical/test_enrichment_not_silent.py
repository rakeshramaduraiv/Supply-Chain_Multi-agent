...
def _make_stubbed_service(monkeypatch, allow_fallback: bool) -> InitializationService:
    ...
    # Stub graph connection — must be AsyncMock so await conn.connect() works
    from unittest.mock import AsyncMock
    graph_conn = AsyncMock()
    graph_conn.connect = AsyncMock()
    graph_conn.disconnect = AsyncMock()
    svc._graph_conn = graph_conn

    # Stub GraphBuilder and EntityExtractor so _graph_steps reaches enrichment
    import app.graph.builder as _builder_mod
    import app.graph.extractor as _extractor_mod
    mock_builder_instance = MagicMock()
    mock_builder_instance.create_constraints = AsyncMock()
    mock_builder_instance.build_full_graph = AsyncMock(return_value=MagicMock(nodes_created=0, relationships_created=0))
    mock_builder_cls = MagicMock(return_value=mock_builder_instance)
    monkeypatch.setattr(_builder_mod, "GraphBuilder", mock_builder_cls)
    mock_extractor = MagicMock()
    mock_extractor.return_value.extract_supplier_routes.return_value = []
    mock_extractor.return_value.extract_routes.return_value = []
    mock_extractor.return_value.extract_inventories.return_value = []
    mock_extractor.return_value.extract_suppliers.return_value = []
    mock_extractor.return_value.extract_products.return_value = []
    mock_extractor.return_value.extract_warehouses.return_value = []
    mock_extractor.return_value.extract_shipments.return_value = []
    mock_extractor.return_value.extract_customers.return_value = []
    mock_extractor.return_value.extract_orders.return_value = []
    mock_extractor.return_value.extract_calendar_events.return_value = []
    mock_extractor.return_value.extract_relationships.return_value = []
    monkeypatch.setattr(_extractor_mod, "EntityExtractor", mock_extractor)
    ...
    monkeypatch.setattr(
        ...
        _raise_enrichment_error_async,
        raising=False,
    )
    # Also patch via the module import path used inside execute()
    import app.graph.enrichment as _enrich_mod
    monkeypatch.setattr(_enrich_mod, "enrich_graph_features_from_neo4j", _raise_enrichment_error_async)

    return svc


async def _raise_enrichment_error(*args, **kwargs):
...
async def _raise_enrichment_error_async(*args, **kwargs):
    from app.graph.enrichment import GraphContextUnavailable
    raise GraphContextUnavailable("Neo4j offline — test-injected failure")
...
def test_enrichment_failure_raises_when_flag_is_false(
    ...
    """
    result = asyncio.run(service_strict.execute(dataset_path=tmp_dataset))
...
def test_enrichment_failure_continues_when_flag_is_true(
    ...
    """
    result = asyncio.run(service_lenient.execute(dataset_path=tmp_dataset))
...
def test_train_all_called_with_graph_enriched_false_on_fallback(
    ...
    """
    asyncio.run(service_lenient.execute(dataset_path=tmp_dataset))
...
def test_train_mask_uses_quantile_boundary(
    ...
    import asyncio
    asyncio.run(service_strict.execute(dataset_path=tmp_dataset))
