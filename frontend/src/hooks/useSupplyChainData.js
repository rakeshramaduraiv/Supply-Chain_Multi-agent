...
export function useDatasetSummary()   { return useQuery({ queryKey: SUPPLY_CHAIN_QUERY_KEYS.datasetSummary,   queryFn: () => api.getDatasetSummary().then(r => r.data),      staleTime: STALE.slow }) }
export function useDatasetAnalytics() { return useQuery({ queryKey: SUPPLY_CHAIN_QUERY_KEYS.datasetAnalytics, queryFn: () => api.getDatasetAnalytics().then(r => r.data),    staleTime: STALE.slow }) }
...
export function useBusinessAlerts() { return useQuery({ queryKey: SUPPLY_CHAIN_QUERY_KEYS.alerts, queryFn: () => api.getBusinessAlerts().then(r => r.data), staleTime: STALE.live, refetchInterval: 30_000 }) }
